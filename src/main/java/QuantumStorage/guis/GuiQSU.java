...
//        if (!this.getInv().getStackInSlot(STORAGE).isEmpty())
//        {
...
//        }
//        if (this.getInv().getStackInSlot(STORAGE).isEmpty()&& !this.getInv().getStackInSlot(OUTPUT).isEmpty())
//        {
//            builder.drawBigBlueBar( this, 31, 43, this.getInv().getStackInSlot(OUTPUT).getCount(), Integer.MAX_VALUE, mouseX - guiLeft, mouseY - guiTop, "Stored", getInv().getStackInSlot(OUTPUT).getDisplayName().toString(),
//                    formatQuantity(this.getInv().getStackInSlot(STORAGE).getCount() + this.getInv().getStackInSlot(OUTPUT).getCount()));
//        }
    }
    ...
    {
        return new ItemStackHandler(this.container.getInventory());
    }
}
