...
public class ContainerQuantumDsu extends RebornContainer {
    ...
    public void detectAndSendChanges() {
        ...
        for (int i = 0; i < this.listeners.size(); i++) {
            ...
            if(tile.storedItem != null && tile.storedItem.stackSize != 0){
                if (this.stackamount != getHStackAmount(tile.storedItem.stackSize, true)) {
                    IContainerListener.sendProgressBarUpdate(this, 0, getHStackAmount(tile.storedItem.stackSize, true));
                }
                if (this.stackSizeType != getHStackAmountType(tile.storedItem.stackSize, true)) {
                    IContainerListener.sendProgressBarUpdate(this, 1, getHStackAmountType(tile.storedItem.stackSize, true));
                }
            } else if(tile.getStackInSlot(1) != null)  {
                if (this.stackamount != getHStackAmount(tile.getStackInSlot(1).stackSize, false)) {
    ...
    public void addListener(IContainerListener crafting) {
        ...
        if(tile.storedItem != null){
            crafting.sendProgressBarUpdate(this, 0, getHStackAmount(tile.storedItem.stackSize, false));
            crafting.sendProgressBarUpdate(this, 1, getHStackAmountType(tile.storedItem.stackSize, false));
        }
    }
    ...
    public int getHStackAmount(int amount, boolean hasExtraItem) {
        if(hasExtraItem){
            amount += 64;
        }
        if (amount <= 1000) {
    ...
    public int getHStackAmountType(int amount, boolean hasExtraItem) {
        if(hasExtraItem){
            amount += 64;
        }
        if (amount <= 1000) {
