...
public class ItemBlockQuantumDsuMk3 extends ItemBlock{

	public ItemBlockQuantumDsuMk3(Block block) {
		super(block);
	}
    ...
    public boolean placeBlockAt(ItemStack stack, EntityPlayer player,  World world, int x, int y, int z, int side, float hitX, float hitY,  float hitZ, int metadata) {
        if (!world.setBlock(x, y, z, ModBlocks.QuantumDsuMk3, metadata, 3)) {
            return false;
        }
        if (world.getBlock(x, y, z) == ModBlocks.QuantumDsuMk3) {
            world.getBlock(x, y, z).onBlockPlacedBy(world, x, y, z, player, stack);
            world.getBlock(x, y, z).onPostBlockPlaced(world, x, y, z, metadata);
        }
        if (stack != null && stack.hasTagCompound()) {
            ((TileQuantumDsuMk3) world.getTileEntity(x, y, z))
