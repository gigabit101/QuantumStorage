...
public class BlockQuantumDsuMk4 extends BlockDsu
{
	...
	public void breakBlock(World world, int x, int y, int z, Block blockId, int meta)
	{
		...
		if(te instanceof TileQuantumDsuMk4)
		{
			if (((TileQuantumDsuMk4) te).getStackInSlot(1) != null){
				...
				ItemStack stacknbt = ((TileQuantumDsuMk4) te).getDropWithNBT();
			...
			else 
			{
				...
				float yOffset = world.rand.nextFloat() * 0.8F + 0.1F;
				float zOffset = world.rand.nextFloat() * 0.8F + 0.1F;
				ItemStack stack = new ItemStack(ModBlocks.QuantumDsuMk4);
