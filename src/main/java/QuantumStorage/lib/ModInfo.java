package QuantumStorage.lib;

public class ModInfo 
{
	public static final String MOD_ID = "QuantumStorage";
	public static final String MOD_NAME = "QuantumStorage";
	public static final String MOD_VERSION = "@VERSION@";
    public static final String MOD_DEPENDENCUIES ="required-after:reborncore";

	public static final String COMMON_PROXY_LOC = "QuantumStorage.proxy.CommonProxy";
	public static final String CLIENT_PROXY_LOC = "QuantumStorage.proxy.ClientProxy";
}
